...
months <- seq(as.Date("2010-12-01"), as.Date("2023-06-01"), by = "month")
...
```{r}
# Step 1: Get distinct towns and asset cities with their coordinates
...
# Step 3: Find the nearest Town for each Asset City
nearest_matches <- distance_results %>%
...
df <- df %>%
  mutate(Year_Month = format(as.Date(Date), "%Y-%m")) %>%  # Convert Date to Date format, then extract year-month
  mutate(Year_Month = as.character(Year_Month))            # Ensure Year_Month is a character

ca <- ca %>%
  mutate(Year_Month = format(as.Date(`Deal Date`), "%Y-%m")) %>%  # Convert Deal Date to Date format, then extract year-month
  mutate(Year_Month = as.character(Year_Month))    

#choose dates between 2010 and june 2023
ca <- ca %>%
  filter(`Deal Date` >= "2010-01-01" & `Deal Date` <= "2023-06-01")
# Step 5: Merge ca and df on Town and Date
merged_data <- ca %>%
...
# save as csv
# write.csv(md, "merged.csv")

nearest_matches %>% arrange(desc(distance))


ca
ca %>% filter(`Asset Cities` == "Needles") %>% count()
