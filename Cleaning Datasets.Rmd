...
# Step 3: Find the nearest Town for each Asset City, and filter by distance <= 50,000
nearest_matches <- distance_results %>%
  group_by(`Asset Cities`) %>%
  slice_min(order_by = distance, n = 1) %>%
  ungroup() %>%
  filter(distance <= 100000) %>%  # Filter to only keep rows with distance <= 50000
...
write.csv(merged_data, "merged_final.csv")
